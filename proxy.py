...
def CheckCachedResponse(command_line, cache):
  ...
  if cmd == "GET":
        if name in cache.myDict:
            #make request to proxy
           return cache.GetValue(name)
        else:
            #go to server and add to cached
            return None
...
def ProxyClientCommand(sock, server_addr, server_port, cache):
  ...
  cmd = library.ReadCommand(sock)
  response = CheckCachedResponse(cmd, cache)
  print("New Response: ", response)

  if response is None:
    command, name, text = library.ParseCommand(cmd)
    response = ForwardCommandToServer(cmd, server_addr, server_port)
    if command == "GET":
      cache.myDict[name] = response
