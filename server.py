...
def PutCommand(name, text, database):
  ...
  ##########################################
  database.myDict[name] = text
  return database.myDict[name]
...
def GetCommand(name, database):
  ...
  # if name in proxyDict:
  #     print("Proxy Call: ")
  #     return str(proxyDict.getValue(name))
  if name in database.myDict:
      return database.myDict[name]
  else:
    val = "Value Not Found"
    return val
...
def DumpCommand(database):
  ...
  for key in database.myDict:
      resultStr = resultStr + database.myDict[key] + ", "
